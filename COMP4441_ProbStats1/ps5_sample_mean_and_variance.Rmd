...
As an application of some of the properties of expected values, this problem set steps through a proof that the expected value of the random variable that defines sample variance is the population variance, given that the population variance is defined. 
...
\begin{align*}
    E\left[\sum_{i=1}^n\left(X_i-\bar X\right)^2\right] &= E\Big[\sum_{i=1}^n\left(X_i^2-2\bar X X_i+x_i^2\right)\Big] \\
    &= E\Big[ \sum_{i=1}^n X_i^2 - \sum_{i=1}^n 2X_i\bar X + \sum_{i=1}^n \bar X^2 \Big] \\
    &= E\Big[ \sum_{i=1}^n X_i^2 \Big] - 2E\Big[\sum_{i=1}^n X_i\bar X \Big] + E\Big[ \sum_{i=1}^n \bar X^2 \Big] \checkmark
\end{align*}
...
If we have random variables $X_i \sim exp(1/3)$, then we can expect $E[X_i^2] = \sigma^2 + \mu^2 = 9 + (3)^2 = 18$. Let's check if our answer is correct.
...
\begin{align*}
    ...
    &= \sum_{i=1}^n \Big( Var[\bar{X}] + E[X]^2 \Big) \\
    &= \sum_{i=1}^n (\frac{\sigma^2}{n} + \mu^2) \\
    &= n (\frac{\sigma^2}{n} + \mu^2)
\end{align*}
...
Because $\bar{X}$ is a constant with respect to the index $i$, we can pull it out of the summation. Then recall $\bar X = \frac{1}{n} \sum_{i=1}^n X_i \rightarrow \sum_{i=1}^n X_i = n\bar{X}$. If we can substitute that into our equation, then we get the resulting $E[n\bar{X}^2]$.
...
\begin{align*}
    ...
    &= n\sigma^2 -2\sigma^2 +\sigma^2 +n\mu^2 -2n\mu^2 +n\mu^2 \\
    &= (n-1)\sigma^2
\end{align*}
...
$$ \boxed{ E\left[\frac{1}{n-1}\sum_{i=1}^n\left(X_i-\bar X\right)^2\right] = \frac{1}{n-1} \Big[(n-1)\sigma^2\Big] = \sigma^2 } $$
